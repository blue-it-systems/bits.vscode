    ...
    const generateDebugConfig = vscode.commands.registerCommand('csharp-test-filter.generateDebugConfig', async () => {
        ...
        try {
            ...
            if (fs.existsSync(launchJsonPath)) {
                ...
                const existingConfig = launchJson.configurations.find((c) => c.name === "Debug TUnit Test");
                if (existingConfig) {
                ...
                for (const input of requiredInputs) {
                    const exists = launchJson.inputs.some(i => i.id === input.id);
                    if (!exists) {
                        launchJson.inputs.push(input);
                    }
                }
            }
            else {
...
async function ensureBreakpointAtMethodEntry(testScopeInfo) {
    const editor = vscode.window.activeTextEditor;
    ...
    if (breakpoints.length === 0) {
        ...
        const breakpoint = new vscode.SourceBreakpoint(new vscode.Location(document.uri, new vscode.Position(braceLineNumber, 0)));
...
async function analyzeTestScopeWithLanguageServer(document, position, showMessages = true) {
...
function findNamespaceFromSymbols(symbols) {
...
function findClassAtPosition(symbols, position) {
    for (const symbol of symbols) {
...
function findMethodAtPosition(classSymbol, position) {
...
function findClassByName(symbols, className) {
    for (const symbol of symbols) {
...
function findMethodByName(classSymbol, methodName) {
    if (!classSymbol.children) {
        return undefined;
    }
    for (const child of classSymbol.children) {
...
async function analyzeTestScope(document, position, showMessages = true) {
...
function extractAssemblyName(filePath) {
...
function findClassName(lines, currentLine) {
...
function findMethodName(lines, currentLine) {
    let braceCount = 0;
    let foundMethod;
    let inMethod = false;
