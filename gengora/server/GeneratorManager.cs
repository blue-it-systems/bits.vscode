...
namespace BITS.Gengora.Server
{
    public class GeneratorManager
    {
        ...
        public GeneratorManager(string workspaceRoot)
        {
        ...
        public async Task<bool> FindAndOpenGeneratorProjectAsync(CancellationToken ct)
        {
            ...
            {
                _generatorProjectPath = envSpecified;
                return true;
            }
            ...
            var csproj = Directory.GetFiles(_workspaceRoot, "Generator.csproj", SearchOption.AllDirectories).FirstOrDefault();
            if (csproj == null)
            {
                var genFolder = Directory.GetDirectories(_workspaceRoot, "Generator", SearchOption.AllDirectories).FirstOrDefault();
                if (genFolder != null)
                {
            ...
            if (csproj == null)
            {
                csproj = Directory.GetFiles(_workspaceRoot, "*Generator*.csproj", SearchOption.AllDirectories).FirstOrDefault();
            }
            ...
            _generatorProjectPath = csproj;
            return true;
        }
        public class BuildResult
        {
            public bool Success { get; set; }
            public List<SimpleDiagnostic> Diagnostics { get; } = new List<SimpleDiagnostic>();
        ...
        public async Task<BuildResult> BuildGeneratorAsync(CancellationToken ct)
        {
            ...
            var projDir = Path.GetDirectoryName(_generatorProjectPath) ?? _workspaceRoot;
            var psi = new System.Diagnostics.ProcessStartInfo
            {
                FileName = "dotnet",
                Arguments = $"build \"{_generatorProjectPath}\" --no-restore --nologo",
            ...
            if (result.Success)
            {
                ...
                if (File.Exists(candidate)) result.BuiltAssemblyPath = candidate;
            }
        ...
        {
            if (string.IsNullOrEmpty(builtAssemblyPath) || !File.Exists(builtAssemblyPath)) return null;
            ...
            try { if (File.Exists(runtimeConfig)) File.Copy(runtimeConfig, Path.Combine(outputDir, Path.GetFileName(runtimeConfig)), overwrite: true); } catch { }
            try { if (File.Exists(deps)) File.Copy(deps, Path.Combine(outputDir, Path.GetFileName(deps)), overwrite: true); } catch { }
            try { if (File.Exists(pdb)) File.Copy(pdb, Path.Combine(outputDir, Path.GetFileName(pdb)), overwrite: true); } catch { }
            return dest;
        }
    }
}
